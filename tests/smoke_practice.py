...
# ---- 题库独立页面 ----
r = client.post("/api/practice/sessions", json={"project_id": "p1"})
data = r.json()
...
r = client.post("/api/practice/answers",
...
r = client.post("/api/practice/answers", json={"session_question_id": sq_ids_a[1], "user_answer": "x"})
assert r.status_code == 200, r.text
res = r.json()
...
# 已作答题目不能重复作答
r = client.post("/api/practice/answers", json={"session_question_id": sq_ids_a[1], "user_answer": "again"})
assert r.status_code == 400, r.text
print("[ok] duplicate answer rejected")

r = client.post(f"/api/practice/sessions/{sid_a}/complete")
assert r.status_code == 200
...
job_b = data["job_id"]
r = client.post(f"/api/practice/generate-job/{job_b}/cancel")
assert r.status_code == 200, r.text
job = r.json()
...
# ---- 会话 B 作答（走 AI 判题降级路径）----
sq_id = sq_ids[0]
r = client.post("/api/practice/answers", json={"session_question_id": sq_id, "user_answer": "x"})
assert r.status_code == 200, r.text
res = r.json()
assert res["status"] == "ok" and 0 <= res["score"] <= 100
...
# ---- 结束会话 ----
r = client.post(f"/api/practice/sessions/{sid_b}/complete")
assert r.status_code == 200
print("[ok] POST /api/practice/sessions/{id}/complete")
